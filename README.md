...
[![Terraform](https://img.shields.io/badge/Terraform-~>%201.7-623CE4?logo=terraform&logoColor=white)](https://www.terraform.io/)
...
Remote State: S3 bucket (umar-tfstate-dev) + DynamoDB table (dynamo-demo)
```
...
|---|---|---|
| `alb_sg` | Internet (0.0.0.0/0) | 80, 443 |
| `client_sg` (EC2) | `alb_sg` only | 80 |
| `db_sg` (RDS) | `client_sg` only | 3306 |
...
Every pull request gets a full infrastructure diff posted as a comment before anyone reviews the code. Sensitive variables (`db_username`, `db_password`) are passed via GitHub Secrets as `TF_VAR_*` environment variables — they never appear in the plan output or the codebase.
...
additional_domain_name  = "app.yourdomain.com"
...
**vpc** — creates the VPC with DNS hostnames enabled, an internet gateway, 2 public subnets across 2 AZs, 4 private subnets across 2 AZs, and a public route table. Uses `data "aws_availability_zones"` so it always picks real AZs for the region — no hardcoding.
...
**route53** — creates a DNS alias record pointing to the CloudFront distribution. Controlled by the `enable_route53` boolean variable so you can deploy the infrastructure without touching DNS.

---

## Things I Would Add Next

- Enable RDS encryption at rest (`storage_encrypted = true`) — currently off, should be on in production
- Add a `terraform.tfvars.example` file with placeholder values so other people can deploy it without guessing variable names
